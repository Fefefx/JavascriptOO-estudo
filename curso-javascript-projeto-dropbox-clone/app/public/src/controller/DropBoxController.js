class DropBoxController {
    ...
    constructor() {
        ...
        this.storageOriginalFolder = ['Hcode'];
        //Cria um evento personalizado
        this.onselectionchange = new Event('selectionchange');
        this.navEl = document.querySelector('#browse-location');
        this.btnSendEl = document.querySelector('#btn-send-file');
    ...
    connectFirebase() {
        ...
        var firebaseConfig = {
            apiKey: "AIzaSyBBF0BExBlovDBBUHad5CRmP3Frpeo8fII",
            authDomain: "dropbox-clone-fbd5d.firebaseapp.com",
            databaseURL: "https://dropbox-clone-fbd5d.firebaseio.com",
            projectId: "dropbox-clone-fbd5d",
            storageBucket: "dropbox-clone-fbd5d.appspot.com",
            messagingSenderId: "762579745355",
            appId: "1:762579745355:web:c750496c6f9e694e542bce",
            measurementId: "G-G4YJMS08QT"
        };
        // Initialize Firebase
            ...
            folderRef.on('value', snapshot => {
                ...
                if(snapshot.exists()){
                    snapshot.forEach(item => {
                        ...
                        data.key = item.key;
                        if (data.type === 'folder') {
                            // CHANGE - data.name para data.originalName
                            this.removeFolderTask(ref + '/' + name, data.originalName).then(() => {
                                resolve({
                                    fields: {
                                        key: data.key
                                    }
                            ...
                            let arrayPath = data.storagePath.split('/');
                            let dataName = arrayPath[arrayPath.length -1];
                            arrayPath.pop();
                            this.removeFile(arrayPath.join('/'), dataName).then(() => {
                                resolve({
                                    fields: {
                                        key: data.key
                                    }
                    ...
                    let keyFolderDatabase = this.getSelection()[0].dataset.key;
                    resolve({
                        fields: {
                            key: keyFolderDatabase
                        }
                    })
                }
                folderRef.remove();
    ...
    removeTask() {
        ...
        this.getSelection().forEach(li => {
            ...
            promises.push(new Promise((resolve, reject) => {
                if (file.type === 'folder') {
                    //CHANGE - this.currentFolder para this.storageOriginalFolder
                    this.removeFolderTask(this.storageOriginalFolder.join('/'), file.originalName).then(() => {
                        resolve({
                            fields: {
                                key
                            }
                ...
                //Trata a referência do Realtime Database que não possui tipo de arquivo
                else if (file.type) {
                    let arrayPath = file.storagePath.split('/');
                    let fileName = arrayPath[arrayPath.length -1];
                    arrayPath.pop();
                    this.removeFile(arrayPath.join('/'), fileName).then(() => {
                        resolve({
                            fields: {
                                key
                            }
    ...
    initEvents() {
        //Adiciona evento da tecla f2 para renomear nome de arquivo
        ...
        //Adiciona o evento da tecla delete para excluir arquivos
        document.addEventListener('keyup',e=>{
            if(e.keyCode === 46){
                if(this.getSelection().length >= 1){
            ...
            this.removeTask().then(responses => {
                responses.forEach(response => {
                    if (response.fields.key) {
                        ...
                        //Desabilitar botões renomear e excluir
        ...
        this.btnRename.addEventListener('click', e => {
            ...
            if (name) {
                file.name = name;
                //Localiza o objeto que tenha a mesma chave do selecionado e altera ele.
        ...
        this.btnSendEl.addEventListener('click', event => {
            this.inputFilesEl.click();
        });
        this.inputFilesEl.addEventListener('change', event => {
            this.btnSendEl.disabled = true;
            //Target seleciona o elemento que disparou o evento
    ...
    uploadComplete() {
        ...
        this.btnSendEl.disabled = false;
    }

    getFirebaseRef(path) {
        if (!path)
            //CHANGE - path = this.currentFolder.join('/');
            path = this.storageOriginalFolder.join('/'); 
    ...
    ajax(url, method = 'GET', formData = new FormData(), onprogress = function () { }, onloadstart = function () { }) {
        return new Promise((resolve, reject) => {
            ...
            /* Com upload, o evento é disparado toda vez que alguma
               porcentagem do arquivo for enviada. */
    ...
    uploadTask(files) {
        ...
        [...files].forEach(file => {
            /*
            //A api FormData lê arquivos 
            let formData = new FormData();
            //Coloca os metadados do arquivo dentro do campo input-file
            ...
            promises.push(new Promise((resolve, reject) => {
                // CHANGE - let fileRef = firebase.storage().ref(this.currentFolder.join('/')).child(file.name);
                let fileRef = firebase.storage().ref(this.storageOriginalFolder.join('/')).child(file.name);
        ...
        //Recebe um array de promessas
        return Promise.all(promises);
    }

    uploadProgress(event, file) {
        let timespent = Date.now() - this.startUploadTime;
        //Quantos bytes do arquivo foram enviados
        let loaded = event.loaded;
        //O tamanho total em bytes do arquivo
        let total = event.total;
        /*Regra de três para calcular porcentagem do arquivo
        seguida de conversão para inteiro, afim de evitar problemas
        com CSS.*/
    ...
    readFiles() {
        //CHANGE - this.lastFolder = this.currentFolder.join('/');
        this.lastFolder = this.storageOriginalFolder.join('/');
    ...
    renderNav() {
        ...
        for (let i = 0; i < this.currentFolder.length; i++) {
            ...
            let originalFolderName = this.storageOriginalFolder[i];
            let span = document.createElement('span');
            //CHANGE - path.push(folderName);
            virtualPath.push(folderName);
            path.push(originalFolderName);
            if ((i + 1) === this.currentFolder.length) {
                ...
                span.innerHTML = `
                        <span class="ue-effect-container uee-BreadCrumbSegment-link-0">
                            <a href="#" data-path="${path.join('/')}" data-virtualpath="${virtualPath.join('/')}" class="breadcrumb-segment">${folderName}</a>
                        </span>
        ...
        this.navEl.querySelectorAll('a').forEach(a => {
            a.addEventListener('click', e => {
                e.preventDefault();
                this.currentFolder = a.dataset.virtualpath.split('/');
                this.storageOriginalFolder = a.dataset.path.split('/');
                this.openFolder();
            });
        });
    }
    ...
    initEventsLi(li) {
        li.addEventListener('dblclick', e => {
            ...
            switch (file.type) {
                case 'folder':
                    ...
                    this.storageOriginalFolder.push(file.originalName);
        ...
        li.addEventListener('click', e => {
            if (e.shiftKey) {
                ...
                //Define o que dispara o evento personalizado
                this.listFilesEl.dispatchEvent(this.onselectionchange);
                return true;
            }
            if (!e.ctrlKey) {
